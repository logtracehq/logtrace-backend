...
type EmailProcessor struct {
	...
	chartRenderer logtrace.ChartRenderer
...
type recipient struct {
	logtrace.UpdateRecipient
	Contact       *logtrace.Contact `json:"contact" bun:"rel:has-one,join:contact_id=id"`
...
func (p *EmailProcessor) acquireProcessingLock(ctx context.Context, updateID uuid.UUID) (bool, error) {
	...
	err := p.db.NewSelect().
		...
		ColumnExpr("TRUE").
		Where("id = ? AND status = ?", updateID, logtrace.UpdateSendScheduleScheduled).
		For("UPDATE SKIP LOCKED").
		Scan(ctx, &locked)
	return locked, err
}

func fetchPendingUpdates(ctx context.Context, db *bun.DB, lastProcessedID string, limit int) ([]*logtrace.UpdateSchedule, error) {
	var scheduledUpdates []*logtrace.UpdateSchedule
	query := db.NewSelect().
		Model(&scheduledUpdates).
		Where("status = ?", logtrace.UpdateSendScheduleScheduled).
		Limit(limit)
...
func (p *EmailProcessor) processUpdate(ctx context.Context, update *logtrace.UpdateSchedule) error {
	...
	if err := updateScheduleStatus(dbCtx, p.db, update, logtrace.UpdateSendScheduleProcessing); err != nil {
	...
	if p.metrics.FailedEmails > 0 {
		if err := updateScheduleStatus(dbCtx, p.db, update, logtrace.UpdateSendScheduleFailed); err != nil {
	...
	return updateScheduleStatus(dbCtx, p.db, update, logtrace.UpdateSendScheduleSent)
}
...
func (p *EmailProcessor) fetchNextBatch(ctx context.Context, updateID uuid.UUID) ([]recipient, error) {
	...
	err := p.db.NewSelect().
		...
		Where("status = ?", logtrace.RecipientStatusPending).
		Relation("Contact").
...
func (p *EmailProcessor) createEmailJobs(recipients []recipient, update *logtrace.Update) []*EmailJob {
	org, err := p.workspaceRepo.Get(context.Background(), &logtrace.FindWorkspaceOptions{
		ID: update.orgID,
	})
	if err != nil {
		// the org is supposed to exist so this is okay to do
		panic(err.Error())
	}

	content, err := prepareEmailTemplate(update, org.WorkspaceName, p.chartRenderer)
	if err != nil {
...
func (p *EmailProcessor) sendEmail(ctx context.Context, job *EmailJob) error {
	...
	if err != nil {
		return updateRecipientStatus(ctx, p.db, p.emailClient, job.Recipient, logtrace.RecipientStatusFailed, "")
	}

	return updateRecipientStatus(ctx, p.db, p.emailClient, job.Recipient, logtrace.RecipientStatusSent, emailID)
}
...
func updateScheduleStatus(ctx context.Context, db *bun.DB, schedule *logtrace.UpdateSchedule,
	status logtrace.UpdateSendSchedule,
) error {
...
func fetchUpdateDetails(ctx context.Context, db *bun.DB, updateID uuid.UUID) (*logtrace.Update, error) {
	update := &logtrace.Update{}
	err := db.NewSelect().
		Model(update).
		Where("id = ?", updateID).
		Scan(ctx)
...
func prepareEmailTemplate(update *logtrace.Update, orgName string, renderer logtrace.ChartRenderer) (string, error) {
	...
	if err := tmpl.Execute(&buf, map[string]string{
		"Content": update.Content.HTML(update.orgID, renderer),
		"Company": orgName,
	}); err != nil {
		return "", err
	}
...
func updateRecipientStatus(ctx context.Context, db *bun.DB, emailClient email.Client, r recipient, status logtrace.RecipientStatus, emailID string) error {
	return db.RunInTx(ctx, &sql.TxOptions{}, func(ctx context.Context, tx bun.Tx) error {
		_, err := tx.NewUpdate().
			Model(&logtrace.UpdateRecipient{}).
		...
		if emailID != "" {
			emailLog := &logtrace.UpdateRecipientLog{
				ProviderID:  emailID,
				RecipientID: r.ID,
				Reference:   logtrace.NewReferenceGenerator().Generate(logtrace.EntityTypeRecipientLog),
		...
		stats := &logtrace.UpdateRecipientStat{
			Reference:   logtrace.NewReferenceGenerator().Generate(logtrace.EntityTypeRecipientStat),
			RecipientID: r.ID,
			HasReaction: false,
			IsDelivered: status == logtrace.RecipientStatusSent,
		}
